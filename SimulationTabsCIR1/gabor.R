...
library("yuima")
dyn.load("gaborlib")
H0 <- 55 #Deinitely pushes the edges out. But creates wide swings --====-- 75 was good!
a <- (1/15)#70 works well for almost all 
b <- (1/15)
...
Obs <- c(5000)
mcoeff <- 0.49 #Less than the exact modulus of continuity.
br <- .05 #Fraction of endpoints to discard
t0 <- 0
T <- 1
K0 <- ceiling((T - t0)/a)+1
...
set.seed(193)
amise <- getmise("cir", cirfunc, Obs,cdata)
