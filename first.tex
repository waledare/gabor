...
\newcommand{\ceen}[1]{\ensuremath{{c}_n(#1, h, k)}\xspace}
\newcommand{\cee}{\ensuremath{{c}(X, h, k)}\xspace}
...
\begin{align}
  g_{h,k}(t) := \mathrm{e}^{2 \pi \i h a t }g(t - k b), \qquad  \forall h,k \in \ints. \notag    
 \end{align}
...
Furthermore, 
\begin{align}
  \tilde{g}_{h,k}(t) := \mathrm{e}^{2\pi\i h a t} \tilde{g}(t - kb), \qquad \forall h, k \in \ints \label{eq:dualghk}
\end{align}
