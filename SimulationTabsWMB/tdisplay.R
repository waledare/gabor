...
getout <- function(out, outfile){
    ...
    cat("\\documentclass{article}\\\\\\begin{document}",t(out),"\\end{document}",file=outfile)
}
getout(out1,"out1.tex")
getout(out2,"out2.tex")

#FIGURES

#X<-data.frame(wmise[[2]])
#dl <- melt(X, id = "Times")
#pw <- ggplot(data=dl, aes(x=Times, y=value, colour=variable)) + geom_line()+ theme_bw() + theme(text=element_text(size=30), plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank() ,panel.border = element_blank())+ theme(axis.line = element_line(color = 'black'), legend.position="bottom", legend.title=element_blank())+ggtitle("Fourier Process") + ylab("Spot variance")
...
#dl <- melt(X, id = "Times")
#pg <- ggplot(data=dl, aes(x=Times, y=value, colour=variable)) + geom_line()+ theme_bw() + theme(text=element_text(size=30),plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank() ,panel.border = element_blank())+ theme(axis.line = element_line(color = 'black'),legend.position="bottom", legend.title=element_blank())+ ggtitle("Geometric Brownian Motion (GBM)") + ylab("Spot (variance)")
